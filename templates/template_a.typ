...
#set page(
  header: align(left)[
    text(font: fonts.heading, weight: "bold", size: 12pt, fill: primary)[#data.company]
  ],
  footer: context align(right)[
    text(font: fonts.body, size: 9pt, fill: accent)[Page #counter(page).display()]
  ]
)
...
#let cover-page(title, subtitle, author, company, date, logo-path) = show {
  ...
  align(center)[
    ...
    text(font: fonts.heading, weight: "bold", size: 30pt, fill: primary)[#title.replace("_", " ")]
    v(0.5cm, weak: true)
    text(font: fonts.body, size: 16pt, fill: primary)[#subtitle]
    v(1fr)
  ]
